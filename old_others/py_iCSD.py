...
ap.add_argument(
    '-wr', metavar = 'weight regularization', type = float, help = 'first derivative regularization weight', default = 1, nargs = '*')
ap.add_argument(
    '-wc', metavar = 'weight constrain', type = float, help = 'current conservation constrain, sum current fractions = 1', default = 10000)
ap.add_argument(
...
def plotCSD(VRTeCoord, VRTeSolution, RealSources = None):
    ...
    if RealSources != None:
        for s in RSs: # add the real sources as red dots
            Rx,Ry = s[0], s[1]
            plt.plot(Rx, Ry,'ow', markersize = 12)
...
def iCSD(A, b, nVRTe, constrain_weight, regularization_weight, nx, ny, verb = 2):
    """ solves the system Ax=b, A and b are given as columns because of the ert code format
        A is reshaped based on nVRTe"""

    A = A.reshape((-1, nVRTe), order = 'F') # reshape by columns (Fortran-like)
    w = np.ones(A.shape[0]) # weight = 1 to all observed data
    ...
    # applied weights with matrix multiplication
    A_crw = np.matmul(W, A_cr)
    b_crw = np.matmul(b_cr, W)
    # solve with lsq_linear
    np.savetxt('Ap.txt', A_crw)
    np.savetxt('bp.txt', b_crw)
    x = lsq_linear(A_crw, b_crw, bounds = (0, 1), verbose = verb)

    return x

def ResidualAnalysis(x, constrain_weight, regularizatio_weight):
    fitting_res = x.fun[0 : nData]
    constrain_res = x.fun[nData + 1] / constrain_weight
    regularization_res = x.fun[nData + 2 :] / w # this constrain is not included in the reg function
    reg_sum = np.sum(np.square(regularization_res)) # ||Ax - b||2
    fit_sum = np.sum(np.square(fitting_res)) # ||x - x0||2
    return fit_sum, reg_sum
...
if __name__ == "__main__":
    ...
    # get number of VRTe in each direction, with mm tollerance
    nx = np.unique(np.round(VRTeCoord[:, 0], 3)).shape[0]
    ny = np.unique(np.round(VRTeCoord[:, 1], 3)).shape[0]
    ...
    if pareto is not None:
        ...
        with PdfPages('iCSD_pareto.pdf') as pdf:
            for w in w_pareto:
                x = iCSD(A, b, nVRTe, constrain_weight, w, nx, ny, verb = 2)
                fr, rr = ResidualAnalysis(x, constrain_weight, regularization_weight)
                pareto_fit_res.append(fr)
                pareto_reg_res.append(rr)
                f = plotCSD(VRTeCoord, x.x, SC)
                pdf.savefig(f)
                plt.close(f)
            f = plt.plot(pareto_fit_res, pareto_reg_res, 'or')
            plt.xlabel('Residual')
            ...
            ax.grid()
            plt.tight_layout()
            plt.savefig('pareto.png', dip = 600)
            pdf.savefig()
        ...
        f = plotCSD(VRTeCoord, x.x, SC)
        f.savefig('iCSD', dpi = 350)
        plt.show()
        plt.plot(b[:204], b[:204] - x.fun[:204], 'or')
        plt.xlabel('Observed [Ohm]')
        plt.ylabel('Simulated [Ohm]')
        plt.grid()
        plt.tight_layout()
        plt.savefig('fitting.png', dpi = 500)
        plt.show()